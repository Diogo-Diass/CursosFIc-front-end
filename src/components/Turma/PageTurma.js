...
function CadTurma() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/instrutor")
      .then((resp) => resp.json())
      .then((retorno_convertido) => setInstrutor(retorno_convertido));
  }, []);

  useEffect(() => {
    fetch("http://localhost:8080/api/horario")
      .then((resp) => resp.json())
      .then((retorno_convertido) => setHorarios(retorno_convertido)); 
  }, []);

  useEffect(() => {
    fetch("http://localhost:8080/api/curso")
      .then((resp) => resp.json())
      .then((retorno_convertido) => setCurso(retorno_convertido));
  }, []);

  useEffect(() => {
    fetch("http://localhost:8080/api/enum/periodo")
      .then((resp) => resp.json())
      .then((retorno_convertido) => setPeriodo(retorno_convertido));
  }, []);
